...
const COINMETRICS_API_URL = "wss://api.coinmetrics.io/v4/timeseries-stream/asset-metrics";
...
if (!API_KEY) {
    console.error("FATAL ERROR: COINMETRICS_API_KEY is not defined in the .env file.");
    process.exit(1);
}
...
const connectToCoinMetrics = () => {
    ...
    const url = `${COINMETRICS_API_URL}?api_key=${API_KEY}&assets=${ASSETS.join(", ")}&frequency=${FREQUENCY}&metrics=${METRICS}`;
    console.log("URL", url);
    const coinmetricsSocket = new ws_1.default(url);
    ...
    coinmetricsSocket.on("message", (data) => {
        try {
    ...
    coinmetricsSocket.on("close", () => {
        console.log(`CoinMetrics connection closed. Reconnecting in ${RECONNECT_DELAY / 1000} seconds...`);
...
const handleCoinMetricsMessage = (data) => {
